...
def main():
    type_ = os.getenv("TYPE_", "dimer")
    spectratype = os.getenv("SPECTRATYPE", "emi")
    spectra_tag = os.getenv("SPECTRA_TAG", "zt")
    max_bonddim = _get_env_int("MAX_BONDDIM", 4)
    evolve_dt = _get_env_float("EVOLVE_DT", 20)
    nsteps = _get_env_int("NSTEPS", 5000)
    ...
    print(
        f"Running {job_name} with TYPE_={type_}, SPECTRATYPE={spectratype}, "
        f"EVOLVE_DT={evolve_dt}, NSTEPS={nsteps}, MAX_BONDDIM={max_bonddim}"
    )
