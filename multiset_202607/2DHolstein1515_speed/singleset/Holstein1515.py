...
"""Singleset MPS speed test for the 2D 15x15 Holstein model.

Parameters follow the 2D g=0.5 entry in Table I of Kloss et al.,
Phys. Rev. Lett. 123, 126601 (2019): omega0=J=1, nu_max=8,
chi0=16, open 15x15 lattice, dt=0.1.
"""
...
OUTPUT_XLSX = os.environ.get(
    "OUTPUT_XLSX",
    f"H{NROW}{NCOL}_2D_s{MAX_BONDDIM}_speed.xlsx",
)
OUTPUT_NPZ = os.environ.get(
    "OUTPUT_NPZ",
    f"H{NROW}{NCOL}_2D_s{MAX_BONDDIM}_speed.npz",
)
...
class LightweightChargeDiffusionDynamics(ChargeDiffusionDynamics):
    """Skip RDM and entropy post-processing for speed benchmarks."""

    def process_mps(self, mps):
        e_occupations = mps.e_occupations
        self.e_occupations_array.append(e_occupations)
        logger.info("e occupations: %s", e_occupations)
...
def main():
    ...
    dynamics_job = LightweightChargeDiffusionDynamics(
        model,
    ...
    logger.info("RDM/k-space/eph-entropy/bond-entropy post-processing disabled for speed test")
    logger.info(
    ...
    logger.info("wall time for evolve: %.6f seconds", wall_time_seconds)
    ...
    np.savez(
        ...
        wall_time_seconds=np.array(wall_time_seconds),
    )
