...
class LE1Dynamics(ChargeDiffusionDynamics):
    def __init__(self, *args, init_state="LE1", **kwargs):
        self.init_state = init_state
        super().__init__(*args, **kwargs)

    def create_electron(self, gs_mp):
        creation_operator = Mpo.onsite(self.model, r"a^\dagger", dof_set=[self.init_state])
...
def run(job_name="p3ht_singleset", max_bond_dim=32, dt_fs=1.0, total_fs=200.0):
    model = P3HTPCBMModel()
    ...
    np.savez(
        ...
        bond_entropy=np.array(job.bond_vn_entropy_array, dtype=object),
    )
    return out_path
...
def main():
    out_path = run(
        job_name=os.environ.get("JOB_NAME", "p3ht_singleset"),
