...
def build_chain_basis_tree(basis_list):
    """Build a linear TTN with the same vibrational order as the multiset MPS."""

    logger.info("Chain TTN vibrational order: %s", [_dof_name(basis) for basis in basis_list])
    return BasisTree.linear(basis_list)
...
def log_chain_shapes(basis_tree, ms_ttns):
    logger.info("Chain TTN basis tree. Electronic states are represented by the multiset set dimension.")
    basis_tree.print(print_function=logger.info)
    logger.info("Chain TTNS shape tree for set 0:")
    ms_ttns.to_ttns_list()[0].print_shape(full=True, print_function=logger.info)
    logger.info("Chain TTN multiset tensor shapes include leading nset dimension:")
    for inode, node in enumerate(ms_ttns.node_list):
        logger.info("node=%d shape=%s", inode, node.tensor.shape)


def run(job_name="p3ht_ms_ttn_chain", max_bond_dim=32, dt_fs=1.0, total_fs=200.0, initial_site=0):
    model = P3HTPCBMModel()
    ...
    basis_tree = build_chain_basis_tree(ms_model.init_model.basis)
    ...
    log_chain_shapes(basis_tree, ms_ttns)
    ...
    logger.info("Chain TTNS shape tree after expansion for set 0:")
    ms_ttns.to_ttns_list()[0].print_shape(full=True, print_function=logger.info)
...
def main():
    print(
        run(
            job_name=os.environ.get("JOB_NAME", "p3ht_ms_ttn_chain"),
