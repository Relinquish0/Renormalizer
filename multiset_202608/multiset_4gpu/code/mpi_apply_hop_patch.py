...
from renormalizer.multiset.multiset_model import _scatter_add_rows
...
def _legacy_pair_sharded_action(self, Y, batched_groups, dim, shape):
    ...
    for group in batched_groups:
        ...
        alpha_sel = group["alpha_idx"][selector]
        ...
        _scatter_add_rows(Y_out, alpha_sel, out_flat)

    # The pair shards are summed below, so the constant Hamiltonian shift must be
    # applied exactly once across the communicator.
    offset = getattr(self, "_energy_offset", 0.0)
    if offset and _CTX.rank == 0:
        Y_out -= offset * Y
...
def _local_electronic_action(self, Y, batched_groups, dim, shape):
    ...
    for group, group_plan in zip(batched_groups, plan["group_plans"]):
        ...
        _scatter_add_rows(
            Y_out, group["alpha_idx"][selector] - alpha_start, out_flat
        )

    # Each rank owns a disjoint set of alpha rows here, so the shift is applied
    # locally and the pieces never overlap.
    offset = getattr(self, "_energy_offset", 0.0)
    if offset:
        Y_out -= offset * Y_local
