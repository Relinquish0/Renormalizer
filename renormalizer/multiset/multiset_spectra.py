            ...
            for beta in range(ket.N_electron):
                for ancilla in range(bra.n_anc):
                    overlaps[alpha, beta] += _state_inner_product(
                        bra.get_electron_ancilla_state(alpha, ancilla),
                        ket.get_electron_ancilla_state(beta, ancilla),
                    )
        return overlaps
...
class MultisetSpectraBase(MultisetTdJob):
    def __init__(
        self,
    ...
    def _record_autocorr_data(self, ft, component_autocorr, mps):
    ...
    def _get_dipole(self) -> np.ndarray:
        ...
        if dipole is None:
            raise ValueError(f"`model.dipole` is required for {type(self).__name__}.")
    ...
    def _expand_ket_bonddim(
        ...
        if reset_configs:
            for component in expanded_ket.msmps:
...
class MultisetSpectraZeroT(MultisetSpectraBase):
    def __init__(
        self,
        model=None,
        spectratype: str = "abs",
    ...
    def process_mps(self, mps):
        ...
        if isinstance(bra, MultisetMps):
            ...
            if self.spectratype == "emi":
                ft = complex(component_autocorr.sum())
            else:
                ft = complex(np.trace(component_autocorr))
        else:
            component_autocorr = np.asarray([[complex(bra.conj().dot(ket))]])
            ft = component_autocorr[0, 0]
        self._record_autocorr_data(ft, component_autocorr, mps)

    def _init_dipole(self):
        dipole_weights = self._get_dipole()
        phi_g = Mps.hartree_product_state(model=self.ms_model.init_model)  # HF state
        phi_g.compress_config = self.ms_model.compress_config
...
class MultisetSpectraFiniteT(MultisetSpectraBase):
    def __init__(
        self,
        model=None,
        spectratype: str = "abs",
        ...
        job_name: str = None,
        expand: bool = True,
        use_electronic_ancilla: bool = False,
    ):
        if temperature == 0:
    ...
    def init_mps_abs(self):
        ...
        if self.expand:
            ket = self._expand_ket_bonddim(ket, preserve_norm=False, reset_configs=True)
    ...
    def _build_ancilla_purification_coefficients(self) -> np.ndarray:
        if not hasattr(self.model, "mol_list") or not hasattr(self.model, "j_matrix"):
            raise ValueError("Electronic ancilla thermal purification requires a Holstein-like model.")
    ...
    def _build_electronic_ancilla_state(self, local_state, weights=None):
        ...
        return ElectronicAncillaMultisetMps(
            ...
            method=self.thermal_init_method,
            msmps=msmps,
            n_anc=self.ms_model.N_electron,
        )
    ...
    def _init_excited_thermal_state(self):
        ...
        if method == "imaginary_time_exact":
            raise ValueError(
                "Finite-temperature excited-state thermal initialisation does not support "
                "`imaginary_time_exact`. Use `imaginary_time_propagate` instead."
            )
        if method != "imaginary_time_propagate":
            raise ValueError(f"Unsupported thermal_init_method: {method}")

        if self.use_electronic_ancilla:
            local_state = self._max_entangled_ground_mpdm(self.ms_model.init_model)
            excited_thermal_state = self._build_electronic_ancilla_state(local_state)
            return self._propagate_excited_thermal_msmpdm(excited_thermal_state)

        msmps = [
            self._max_entangled_ground_mpdm(self.ms_model.MsModel[alpha][alpha])
            for alpha in range(self.ms_model.N_electron)
        ]
        excited_thermal_state = self._build_multiset_state(msmps)
        return self._propagate_excited_thermal_msmpdm(excited_thermal_state)
        ...
        if getattr(state, "electronic_ancilla", False):
            ...
            for alpha in range(state.N_electron):
                for ancilla in range(state.n_anc):
                    component = state.get_electron_ancilla_state(alpha, ancilla).copy()
    ...
    def _evolve_finite_temperature_branch(self, state, evolve_dt):
        if not isinstance(state, MultisetMps):
            raise TypeError(
                f"Finite-temperature spectra branches should be multiset states. Got {type(state)}."
            )

        msmps = []
        for mpdm in state.msmps:
            msmps.append(self._evolve_ground_mpdm(mpdm, -evolve_dt))
        state = self._build_multiset_state(msmps)
