...
def expm_krylov(Afunc, dt, vstart: xp.ndarray, block_size=50):
    ...
    for j in range(len(vstart)):
        ...
        if 3 < j and j % 2 == 0:
            ...
            if res is not None and xp.allclose(res, new_res, rtol=1e-8, atol=1e-10 ): # rtol=1e-8, atol=1e-10
                # Default rtol=1e-5, atol=1e-8. Lower tolerance provides higher precision, but requires more memory.
                return new_res, j+1
            else:
                res = new_res
        V[j + 1] = w / beta[j]